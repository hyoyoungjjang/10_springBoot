...
//@Entity : 해당 클래스가 데이터베이스의 데이블과 매핑되는 엔티티클래스임을 나타낸다. -> 테이블로 변환된다.
...
@Data
public class Board {
    @Id // 엔티티의 기본기(Primary Key)임을 나타냄
    ...
    @Column(columnDefinition = "TEXT")//해당 컬럼은 TEXT타입으로 명시, 일반적으로 생성시 varchar(225)
    ...
    @UpdateTimestamp // 엔티티가 업데이트될때마다 타임스탬프를 자동으로 갱신
    @Column
